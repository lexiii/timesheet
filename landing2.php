...
<?
        echo "<h3 class='text-uppercase'><small>Clocked in ".relativeTime(strtotime($row['startTime']))."</small></h3>";
        echo "<table class='one-row table'>";
        $startTime = strtotime($row['startTime']);
        $startTime = "<p class='date'>".date("m/d/Y",$startTime)."</p><p class='time'>".date("H:i",$startTime)."</p>";
        if(isset($row['endTime'])){
            $endTime = strtotime($row['endTime']);
            $endTime = "<p class='date'>".date("m/d/Y",$endTime)."</p><p class='time'>".date("H:i",$endTime)."</p>";
            //            $endTime = date("m/d/Y",$endTime)."<br/>".date("H:i",$endTime);
        }else{
            $endTime = "<p class='text-uppercase'><strong>Still Clocked In</strong></p>";
        }
        echo "<tr>".
            "<td>".
            $startTime.
